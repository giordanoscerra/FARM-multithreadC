...
valgrind --error-exitcode=1 --log-file=/dev/null ./farm -d testdir file*
if [[ $? != 0 ]]; then
    echo "                                test4 failed"
else
    echo "test4 PASSED"
...
valgrind --leak-check=full --error-exitcode=1 --log-file=/dev/null ./farm file* -d testdir
if [[ $? != 0 ]]; then
    echo "                                test5 failed"
else
    echo "test5 PASSED"
...
# verifica di memory leaks
valgrind --leak-check=full --error-exitcode=1 ./farm -d miniTest -n 1 -q 1
if [[ $? != 0 ]]; then
    echo "                                test6 failed"
else
    echo "test6 PASSED"
